...
<div class="recentPostContainer overflow-hidden">
    ...
    <div class="row">
        <div class="breadcrumb">
            <a href="<?php echo URL::to('/'); ?>">{{__('labels.home')}}</a> >
            @foreach($parents as $parent)
                @if($parent->Id != $categ_id) <a href="<?php echo URL::to('/'); ?>/caregory/{{$parent->Id}}">{{$parent->Name}}</a> > @else {{$parent->Name}} @endif
            @endforeach
        </div>
    </div>
    <div class="row">
                    ...
                    <article class="u-whiteBg u-shadow-0x0x5--05 u-flex u-flex--contentSpace u-flex--dir_col">
                        <div class="__flex-top">
                            <h3 class="categoryPost__title u-fontWeightBold u-marginBottom5">
                                <a class="textDark" href="<?php echo URL::to('/'); ?>/article/{{$art->article_id}}">{{substr($art->title,0,80)}}...</a>
                            </h3>
                        ...
                        <footer class="categoryPost__footer clear u-paddingTop15 n-magrinBottom6">
                            <a class="u-relative u-fontWeight600" href="<?php echo URL::to('/'); ?>/article/{{$art->article_id}}">{{__('labels.read_more')}}</a>
                        </footer>
                    </article>
                </div>
            @endforeach
