                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body pb-2 ">
                            <form class="needs-validation" method="post" action="<?php echo URL::to('/'); ?>/admin/category/edit/{{$categ->Id}}">
                                @csrf
                                <div class="row row-sm">
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <input class="form-control" placeholder="Name" name="name" required="required" type="text" value="{{$categ->name}}">
                                        </div>
                                        <div class="form-group">
                                            <label class="form-label">Status</label>
                                            <select name="status" id="select-countries" placeholder="E-Mail" class="form-control custom-select select2">
                                                <option value="1" @if($categ->status == 1) {{'selected="selected"'}} @endif >Active</option>
                                                <option value="0" @if($categ->status == 0) {{'selected="selected"'}} @endif >Inactive</option>
                                            </select>
                                        </div>
